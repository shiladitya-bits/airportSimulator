/*
Driver file. 
Immigration could not be implemented properly. I have commented out the code that i wrote for it. I hope the logic is considered.
When PASSPORT validation fails, it works properly. I am having problems when VISA is called if PASSPORT is valid.
*/
#include "header.h"
passenger *plist;
int num;
pid_t bcounter, immigration, checksec, lounge, boarding,cafe,rest,boardpass,lugsec,weight,secm,secf,passport,visa;
int cafepipe[2],restpipe[2];
char cafebuf[15],restbuf[15];
int i=0,loungers=0;
void sigtoStop()
{
	signal(SIGUSR2,sigtoStop);
	kill(lounge,SIGCONT);
	sleep(3);
	printf("Total number of passengers arrived at airport= %d\n",num);
	printf("Total number of passengers boarded = %d\n",num); //change this
	printf("Flight Number #%d ready for departure with %d passengers.\n",getpid(),num);
}
void gotorest()
{
	signal(SIGUSR1,gotorest);
	read(restpipe[0],restbuf,15);
	//printf("Passenger %s entered Cafe 1\n",plist[i].flt.ticket_no);
	printf("Passenger %s entered Cafe 1\n",restbuf);
	
	kill(boarding,SIGUSR1);
	sleep(1);
	kill(boarding,SIGUSR2);
}
void addLoungers()
{
...
void gotocafe()
{
	...
	read(cafepipe[0],cafebuf,15);
	//printf("Passenger %s entered Restroom\n",plist[i].flt.ticket_no);
	printf("Passenger %s entered Restroom\n",cafebuf);
